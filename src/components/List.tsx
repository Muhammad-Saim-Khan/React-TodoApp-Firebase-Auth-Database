...
export default function List() {
  ...
  const mapQuerySnapshotToTasks = (querySnapshot: QuerySnapshot<any>): Task => {
    return querySnapshot.docs.map((doc: QueryDocumentSnapshot<any>) => {
      const data = doc.data();
      return {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      query(docRef, where("completed", "==", false)),
      (q) => {
    ...
    <div className="relative my-4">
      ...
      <footer>
        <div className="w-full footer-color font-bold py-4 px-4 flex justify-between">
          <p ref={elementOneRef} className="text-sm text-white font-bold">
            {completedTasksCount} items lefts
          </p>
          <div
            ref={elementTwoRef}
          ...
          >
            <p
              ...
              className={`${
                activeState === "All" ? "active-color" : ""
              } list-footer-text cursor-pointer text-sm text-black  font-bold`}
            >
              All
            </p>
            <p
              className={`${
                activeState === "Active" ? "active-color" : ""
              } list-footer-text cursor-pointer text-sm text-black  font-bold`}
            ...
            <p
              className={`${
                activeState === "Completed" ? "active-color" : ""
              } list-footer-text cursor-pointer text-sm text-black  font-bold`}
          ...
          <p
            ref={elementThreeRef}
            className="list-footer cursor-pointer text-sm text-black font-bold "
